...
func TestEtcd(t *testing.T) {
	...
	etcdClinet, err := NewEtcd(etcdCfg)
	if err != nil {
	...
	if err = etcdClinet.PutRegisterService("/host/mysql_host", "10.171.5.216:3306"); err != nil {
		t.Errorf("Mysql put err: %v.", err)
		return
	}

	if err = etcdClinet.PutRegisterService("/host/redis_host", "10.171.5.216:6382"); err != nil {
		t.Errorf("Redis put err: %v.", err)
		return
	}

	if err = etcdClinet.PutRegisterService("/host/mqtt_host", "10.171.5.216:1883"); err != nil {
	...
	for {
		count++
		service, err := etcdClinet.GetDiscoverService("*")
		if err != nil {
