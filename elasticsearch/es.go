...
func NewElastic(cfg *ElasticConfig) (*Es, error) {
...
func (c *Es) GetClient() *elastic.Client {
	return c.cli
}

// InsertDoc 插入doc
func (c *Es) InsertDoc(indexName, docId string, doc interface{}, ctx context.Context) (*elastic.IndexResponse, error) {
...
// BatchInsertDocs 批量插入doc
func (c *Es) BatchInsertDocs(indexName string, docIds []string, docs []interface{}, ctx context.Context) (*elastic.BulkResponse, error) {

	if len(docIds) != len(docs) && len(docIds) != 0 {
		return nil, errors.New("[es]len docIds and docs is not equal")
...
// DeleteDoc 删除doc
func (c *Es) DeleteDoc(indexName, docId string, ctx context.Context) (*elastic.DeleteResponse, error) {
...
// DeleteDocsByConditions 删除doc（存在筛选条件）
...
func (c *Es) DeleteDocsByConditions(indexName string, condition DeleteDocByCondsParam, ctx context.Context) (*elastic.BulkIndexByScrollResponse, error) {
	...
	if len(condition.RangeConditions) > 0 {
		for rangeConditionK, rangeConditionV := range condition.RangeConditions {
			if len(rangeConditionV) != 2 {
				return nil, errors.New("[es]rangeConditions length is not 2")
			} else {
...
// UpdateDoc 更新doc
func (c *Es) UpdateDoc(indexName, docId string, doc interface{}, ctx context.Context) (*elastic.UpdateResponse, error) {
...
// BatchUpdateDocs 批量更新doc
func (c *Es) BatchUpdateDocs(indexName string, docIds []string, docs []interface{}, ctx context.Context) (*elastic.BulkResponse, error) {
	if len(docIds) == 0 {
		return nil, errors.New("[es]len docIds is 0")
...
// UpdateDocsByConditions 更新doc（存在筛选条件）
...
// 待筛选的key 必须是原子类型，比如keyword/int等格式，text 格式可在分，所以不建议检索（分词可能不精准包括）
func (c *Es) UpdateDocsByConditions(indexName string, scripts []string, condition UpdateDocByCondsParam, ctx context.Context) (*elastic.BulkIndexByScrollResponse, error) {
	if len(scripts) == 0 {
		return nil, errors.New("[es]scripts is empty")
	}
...
func (c *Es) UpsertDoc(indexName, docId string, doc interface{}, ctx context.Context) (interface{}, error) {
	if docId == "" {
		// 不存在，执行插入操作
...
// BatchUpsertDocs 批量更改（更新或插入）doc
func (c *Es) BatchUpsertDocs(indexName string, docIds []string, docs []interface{}, ctx context.Context) (*elastic.BulkResponse, error) {
...
// QueryDoc 查询doc
func (c *Es) QueryDoc(indexName, docId string, ctx context.Context) (*elastic.GetResult, error) {
	// 根据id 查询文档
...
// QueryDocsByConditions 查询doc（存在筛选条件）
...
// 待筛选的key 必须是原子类型，比如keyword/int等格式，text 格式可在分，所以不建议检索（分词可能不精准包括）
func (c *Es) QueryDocsByConditions(indexName string, condition QueryDocByCondsParam, sortField string, sortByASC bool, from, size int, ctx context.Context) (*elastic.SearchResult, error) {
	// 创建bool 查询
	boolQuery := elastic.NewBoolQuery().Must()
...
// BatchQueryDocs 批量查询doc
func (c *Es) BatchQueryDocs(indexName string, docIds []string, ctx context.Context) (*elastic.MgetResponse, error) {
	if len(docIds) == 0 {
		return nil, errors.New("[es]len docIds is 0")
	}
...
func (c *Es) DeleteIndexes(indexNames []string, ctx context.Context) (*elastic.IndicesDeleteResponse, error) {
...
// DeleteIndex 删除单个索引
func (c *Es) DeleteIndex(indexName string, ctx context.Context) (*elastic.IndicesDeleteResponse, error) {
...
// ExistIndex 索引是否存在
func (c *Es) ExistIndex(indexName string, ctx context.Context) (bool, error) {
