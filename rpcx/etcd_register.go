...
func (c *EtcdConf) Check() error {
	...
	if len(c.Endpoints) == 0 {
		return errors.New("Etcd miss endpoints.")
	}
...
func NewServiceRegister(cfg *EtcdConf, serName, addr string) (*ServiceRegister, error) {
	...
	if err != nil {
		logrus.Errorf("New service register err: %v.", err)
	}
	...
	logrus.Infof("New service register success, `endpoints`: %v, `serName`: %s. `ttl`: %d.", cfg.Endpoints, serName, cfg.TTL)
	return ser, nil
}
...
func (s *ServiceRegister) putKeyWithLease(ttl int64) error {
	...
	logrus.Infof("Put key:%s, val:%s success!", s.key, s.val)
	...
	for {
		...
		case leaseKeepResp := <-s.keepAliveChan:
			if leaseKeepResp == nil {
				logrus.Infof("Lease '%d' has been closed.", s.leaseID)
				return
			} else {
				logrus.Infof("Lease '%d' success, detail: %+v.", s.leaseID, leaseKeepResp)
			}
		}
	}
}
